...
/// DYNAMIC LOAD GENERATOR ///
// 1=> 1:99
int mix_num = 9; //11; //9;	//choose the traffix mix from above traffic_options -> {0,1,2}  // 11 means only attach request will be sent
float a_prob = 0;
float s_prob = 0;
	...
	while(curTime < endTime){
		...
		switch(traffic_type){
			...
			case 4: setUpTunnel_t = true;
					...
					loop2 = 99; //9999999;   //inner loop---service-req
					break;
...
int main(int argc, char *args[]){
	...
	if(sendData || networkServiceRequest){
		...
		// Client user(1);
		//user.input_server_details(g_mme_port, g_mme_address);
		string startingIp1 = "10.127.0.1"; //get_starting_IP_Address(ue, user);
