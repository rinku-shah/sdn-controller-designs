...
public class EpcApp {
   ...
   /* @Reference(cardinality = ReferenceCardinality.MANDATORY_UNARY)
    ...
    /*private ConsistentMap<String, String> exmap;
	...
	////////////////
      ...
      log.warn("Connection to server sucessfully");*/
      ...
      for(int i = 0; i<list.size(); i++) {
         log.warn("List of stored keys:: {}", list.get(i));
      }
        jedis.set("First","1");
        jedis.set("Second", "2");
        log.warn("Get: \"First\"= {} ", jedis.get("First"));
        jedis.del("First");

        List<String> listnew = convertSetToList(jedis.keys("*"));
        for(int i = 0; i<listnew.size(); i++) {
           log.warn("List of stored keys:: {} ", listnew.get(i));
        }*/



/*
	exmap = storageService.<String, String>consistentMapBuilder()
...
*/
         ...
         */
/*        exmap.addListener(listener);
        log.info("Started");
	log.info("Consistent map successfully listening");
	for(int i=1; i<=10; i++){
		exmap.put(Integer.toString(i), Integer.toString(i));
		String s = exmap.get(Integer.toString(i)).value(); 
    ...
    private void requestIntercepts() {
        ...
        // @traditional-bitwise design : we will request context release and service request packets as well at root
...
//                log.info("context is already handled");
			...
			kv.put("uekey_sgw_teid_map"+tmpArray[2], Constants.getSgwDpid(dgw_dpId) + Constants.SEPARATOR + tmpArray2[1]);

                        

                        //FT.put(Integer.parseInt(Constants.SEND_APN),dgw_dpId, "sgw_teid_uekey_map", tmpArray2[1], tmpArray[2]);
			kv.put("sgw_teid_uekey_map"+ tmpArray2[1], tmpArray[2]);
			...
			String tmp = kv.get("uekey_sgw_teid_map"+tmpArray[2]); // tmpArray[2] => ue key
			...
			kv.del("uekey_sgw_teid_map"+tmpArray[4]);// newly added.. because can't remove in SEND_UE_TEID step.. due to re-establishment of tunnel
			...
			kv.del("sgw_teid_uekey_map"+tmpArray[3]); // key is sgw teid and value is ue key
			...
			String sgw_dpid_sgw_teid = kv.get("uekey_sgw_teid_map"+tmpArray[1]); // MAP key = UE KEY,  MAP value = SGW_DPID + SEPARATOR + SGW_TEID
                        ...
                        response = new StringBuilder();
                        response.append(Constants.INITIAL_CONTEXT_SETUP_REQUEST).append(Constants.SEPARATOR).append(sgw_teid).append(Constants.SEPARATOR);
			...
			tmp = kv.get("uekey_sgw_teid_map"+tmpArray[2]); // tmpArray[2] => ue key
                        tmpArray2 = tmp.split(Constants.SEPARATOR);
                        //tmpArray2[0] => sgw_dpId  and tmpArray2[1] => sgw_teID
...
////////////////////////////////
 ...
 /*
     ...
     * its event method.
     */
   /* private class InternalListener implements MapEventListener<String, String> {
        @Override
