...
#rt=`sudo lxc-info -p -n onos2-controller|tr -d ' '|cut -d ':' -f2`
...
pg=`sudo lxc-info -p -n pgw-horiz|tr -d ' '|cut -d ':' -f2`
...
#sh cpu_util.sh $1 $2 $rt controller_util.txt tmp_c $3 "^%Cpu0 \|^%Cpu1 \|^%Cpu2 \|^%Cpu3 " &
...
sh cpu_util.sh $1 $2 $sg3 sgw3_util.txt tmp_s3 $3 "^%Cpu16 \|^%Cpu17 " &

sh cpu_util.sh $1 $2 $sg4 sgw4_util.txt tmp_s4 $3 "^%Cpu18 \|^%Cpu19 " &
